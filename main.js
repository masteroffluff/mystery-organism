...
function pAequorFactory(specimenNum, dna){
  ...
  return {
    ...
    mutate(){
      let randomBaseID = Math.floor(Math.random() * this.dna.length);
      let newBase;
      do{
        const dnaBases = ['A', 'T', 'C', 'G'];
        newBase= dnaBases[Math.floor(Math.random() * 4)];
      } while (newBase === this.dna[randomBaseID]);
      this.dna[randomBaseID] = newBase;
      return this.dna;
    },
    ...
    compareDNA(pAequor){
        ...
        let percentMatch = (matchCount/this.dna.length)*100
...
// tests on wild pAequors with random dna to make sure this works for reals
...
collectionOf30.forEach(item=>console.log(item)) // prinitng like this because VSC does not like to print lots of arrays on the console
...
// commented out for now as as is will fill the console and obsure the other results. 
